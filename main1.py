...
from data1 import HELLO_TEXT, BOT_INFO_TEXT, DONATE_BOT_TEXT, SEND_BACKMESSAGE_TEXT
#####################################################################################
import requests
    ...
    while retries < max_retries:
        try:
            ...
            if retries < max_retries:
                retries += 1
                wait_time = 2**retries
                logging.info(f'Пытаемся повторно подключиться через {wait_time} сек.')
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, text=HELLO_TEXT.format(message.from_user), reply_markup=markup)
############################################################################################################################
